...
test("OpenAI --print 无取消信号时仍保留 SDK 重试", (t) => {
  const testHome = mkdtempSync(join(tmpdir(), "howto-provider-print-retry-"));
  try {
    const child = spawnSync(process.execPath, [CHILD_ENTRYPOINT, "print-retry"], {
      env: { HOME: testHome, PATH: "/usr/bin:/bin" },
      cwd: testHome,
      encoding: "utf8",
      timeout: 5_000,
      killSignal: "SIGKILL",
    ...
    assert.match(child.stdout, /^ls\n/);
    const records = [...child.stdout.matchAll(/HOWTO_PROVIDER_RESULT:([^\n]+)/g)].map(
      (match) => JSON.parse(match[1]) as Record<string, unknown>,
    );
    assert.equal(records.length, 2);
    assert.deepEqual(
      records.map(({ event, exitCode, requestCount, abortCount }) => ({
        event,
        exitCode,
        requestCount,
        abortCount,
      })),
      ["returned", "exit"].map((event) => ({
        event,
        exitCode: 0,
        requestCount: 2,
        abortCount: 0,
    ...
    t.diagnostic(`Node ${process.versions.node}: ${JSON.stringify(records)}`);
  } finally {
    rmSync(testHome, { recursive: true, force: true });
  }
});
